...
class NSIDC_area:
	...
	def masksload(self):
		...
		'''
		filename = f'/home/nico/Cryoscripts/AMSR/Masks/AMSR2_{self.hemi}_land.npz'
		self.landmask = CryoIO.readnumpy(filename).flatten()
		
		filename = f'/home/nico/Cryoscripts/AMSR/Masks/AMSR2_nh_pole_ring.npz'
		self.polering = CryoIO.readnumpy(filename).flatten()

		self.maskview(self.polering)
		plt.show()
	...
	def dayloop(self):
		...
		for count in range (0,self.daycount,1):
			...
			if self.hemi == 'nh':
				filepath = f'/home/nico/Cryoscripts/AMSR/DataFiles/north/{loopday.year}/'
			else:
				filepath = f'/home/nico/Cryoscripts/AMSR/DataFiles/south/{loopday.year}/'
...
# =============================================================================
	...
	def maskview(self,icemap):
		...
		plt.imshow(icemap, interpolation='nearest', vmin=0, vmax=1, cmap=plt.cm.jet)

		
	def exportdata(self):
		CryoIO.csv_columnexport(f'temp/AMSR2_test_{self.hemi}.csv',
