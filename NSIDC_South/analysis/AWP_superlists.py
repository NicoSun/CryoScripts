...
class Data_restructure:
    ...
    def createSuperRegionList (self):
        ...
        for year in range(1980,2023):
            AWP_mean = ['A', 'B', 'C', 'D', 'E']
            Climatedata = pd.read_csv(f'AWP/csv/_AWP_regional_{year}_south.csv', names=AWP_mean,header=0)
...
#            self.appendSuperRegionList(data)
        ...
        self.csvexport('AWP/AWP_region_Heatmap_south.csv',[self.region1,self.region2,self.region3,self.region4,self.region5])
    ...
    def regionExcelbook_whole_Antarctic (self):
        ...
        names = ['Daily','Accu']
        inputfile = 'AWP/csv/_AWP_{}_south.csv'
        outputfile = 'AWP/AWP_new.xlsx'
...
#            print(Climatedata.B[0])
    ...
    def regionExcelbook_Daily_by_year (self):
        with pd.ExcelWriter('AWP/AWP_by_Year_regional_Daily.xlsx') as writer:
            for year in range(1980,2023):
                Climatedata = pd.read_csv('AWP/csv/_AWP_regional_daily_{}_south.csv'.format(year))
                Climatedata.to_excel(writer, sheet_name=str(year),index=False)
                
    def regionExcelbook_Accu_by_year (self):
        with pd.ExcelWriter('AWP/AWP_by_Year_regional_Accu.xlsx') as writer:
            for year in range(1980,2023):
                Climatedata = pd.read_csv('AWP/csv/_AWP_regional_{}_south.csv'.format(year))
                Climatedata.to_excel(writer, sheet_name=str(year),index=False)
...
#        
        if mode == 'Accu':
            inputfile = 'AWP/csv/_AWP_regional_{}_south.csv'
            outputfile = 'AWP/AWP_by_region.xlsx'
        elif mode == 'Daily':
            inputfile = 'AWP/csv/_AWP_regional_daily_{}_south.csv'
            outputfile = 'AWP/AWP_by_region_Daily.xlsx'
...
#            print(Climatedata.B[0])
    ...
    def regionExcelbook_by_region_decades (self,mode):
        ...
        if mode == 'Accu':
            inputfile = 'AWP/csv/_AWP_regional_{}_south.csv'
            outputfile = 'AWP/AWP_by_region_decades.xlsx'
        elif mode == 'Daily':
            inputfile = 'AWP/csv/_AWP_regional_daily_{}_south.csv'
            outputfile = 'AWP/AWP_by_region_daily_decades.xlsx'
        ...
        if mode == 'decades':
            inputfile = 'AWP/csv/_AWP_regional_{}_south.csv'
            outputfile = 'AWP/2020s_mean_AWP_by_region.xlsx'
            
#            inputfile = 'AWP/csv/_AWP_regional_daily_{}.csv'
#            outputfile = 'AWP/2020mean_AWP_by_region_Daily.csv'
...
mode = 'decades' #Daily, Accu, decades

action = Data_restructure()
if __name__ == "__main__":
     # action.createSuperRegionList()
     # action.regionExcelbook_whole_Antarctic()
