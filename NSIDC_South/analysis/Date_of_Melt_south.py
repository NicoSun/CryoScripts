...
class NSIDC_area:
	...
	def __init__  (self):
		self.start = date(2011, 9, 25)
		self.month = self.start.month
		self.day = self.start.day
		self.daycount = 155 #165 (Oct-Feb)
		self.decade = 1980
	...
	def dayloop(self):
		...
		for year in range(self.decade,self.decade+10):
			filename = 'DataFiles/NSIDC_{}0925_south.bin'.format(year)
	...
	def meltdate(self,iceminyear,icemeanyear,icemaxyear,icemin,icemedian,icemax,regmask):
		...
		if regmask < 10:
			if icemin < 37 and iceminyear == 11:
				iceminyear =  self.day_of_year
			if icemedian < 37 and icemeanyear == 11:
				icemeanyear = self.day_of_year
			if icemax < 37 and icemaxyear == 11:
	...
	def normalshow(self,icemap,code):
		...
		cax = ax.imshow(map1, interpolation='nearest', vmin=274, vmax=426, cmap=cmap)
	...
	def combograph (self,early,median,late):
		...
		fig.suptitle('Antarctic Melt Days',fontweight='bold',fontsize=14)

		ax1.set_title('Earliest Melt Date')
		ax1.imshow(map1, interpolation='nearest', vmin=274, vmax=426, cmap=cmap)
		ax1.imshow(map11, interpolation='nearest', vmin=-1, vmax=1, cmap=plt.cm.Greys)
		
		ax2.set_title('Median Melt Date')
		ax2.imshow(map2, interpolation='nearest', vmin=274, vmax=426, cmap=cmap)
		ax2.imshow(map21, interpolation='nearest', vmin=-1, vmax=1, cmap=plt.cm.Greys)
		
		ax3.set_title('Latest Melt Date')
		cax = ax3.imshow(map3, interpolation='nearest', vmin=274, vmax=426, cmap=cmap)
		...
		ax3.text(0.02, 0.02, 'cryospherecomputing.com',transform=ax1.transAxes)
		
		
		fig.tight_layout(pad=-3)
		
		cbar = fig.colorbar(cax, ticks=[274,305,335,366,397,426],cmap=cmap)
		cbar.ax.set_yticklabels(['Oct','Nov','Dec','Jan', 'Feb', 'Mar'])
		...
		fig.savefig('csvexport/Combo_South_Melt_Date.png')
