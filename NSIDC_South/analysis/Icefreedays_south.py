...
class NSIDC_IcefreeDays:
	...
	def __init__  (self):
		self.root = '/run/media/nico/Cryosphere/NSIDC_South/'
		self.masksload()
		
		
	def masksload(self):
		regionmaskfile = f'{self.root}Masks/region_s_pure.msk'
		self.regmask = CryoIO.openfile(regionmaskfile, np.uint8)
...
# 		filepath = '/home/nico/Cryoscripts/NSIDC_South/DataFiles/'
		filepath = '/run/media/nico/Cryosphere/NSIDC_South/DataFiles/'
...
# 		plt.show()
	...
	def normalshow(self,icemap,year):
		...
		ax.set_title('Antarctic Ice Free Days: {}'.format(year),x=0.5)
		...
		fig.savefig(f'{self.root}temp/images/Antarctic_IceFreeDays_{year}.png')
		plt.close()
	...
	def anomalyshow(self,icemap,year):
		...
		fig.savefig(f'{self.root}temp/images/Antarctic_IceFreeDays__anom_{year}.png')
		plt.close()
