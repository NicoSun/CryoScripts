...
# 			filename = f'DataFiles/{self.mode}/NSIDC_{self.mode}_{stringmonth}{stringday}_south.bin'
...
#  			filename = f'temp/NSIDC_{self.mode}_{stringmonth}{stringday}_south'
# =============================================================================
			
			xlabel = f'Date: {self.year}/{stringmonth}/{stringday}'
			filename = f'temp/NSIDC_{self.year}{stringmonth}{stringday}_south.png'
...
# 		plt.show()
	...
	def dailyloop(self,icemap,area,extent, xlabel,filename):
		...
		self.ax.text(0.35, 0.45, 'Area: '+'{:,}'.format(area)+' 'r'$km^2$', fontsize=10,color='white',transform=self.ax.transAxes)
		self.ax.text(0.35, 0.42, 'Extent: '+'{:,}'.format(extent)+' 'r'$km^2$', fontsize=10,color='white',transform=self.ax.transAxes)
		
		self.ax.text(2, 8, 'cryospherecomputing.tk', fontsize=8,color='white',fontweight='bold')
		self.ax.text(210, 8, 'data source: NSIDC', fontsize=8,color='white',fontweight='bold')
...
#			self.cbar = self.fig.colorbar(self.cax, ticks=[0,25,50,75,100]).set_label('Ice concentration in %')
		self.cbar = self.fig.colorbar(self.cax, ticks=[0,25,50,75,100],shrink=0.88).set_label('Ice concentration in %')
