...
class NSIDC_Filler:
	...
	def dayloop(self):
		...
		for count in range (0,self.daycount,1): 
			...
			filenamePlus1 = 'DataFiles/NSIDC_{}_south.bin'.format(self.calcday(1))
			
			with open(filenamePlus1, 'rb') as fr:
				iceP1 = np.fromfile(fr, dtype=np.uint8)
			
			icechange = np.zeros(len(ice))
			...
			export = np.array(icechange, dtype=np.uint8) 
			self.dailyloop(export)
			filename_out = 'SIPN_init_{}{}{}_south.bin'.format(self.year,self.stringmonth,self.stringday)
...
#		self.cax = self.ax.imshow(icemap, interpolation='nearest', vmin=0, vmax=250, cmap=cmap)
	...
	def dailyorcumu(self):		
		...
		self.cax = self.ax.imshow(self.icenull, interpolation='nearest', vmin=0, vmax=250,cmap = plt.cm.jet)
#			self.cbar = self.fig.colorbar(self.cax, ticks=[0,25,50,75,100]).set_label('Ice concentration in %')
		self.cbar = self.fig.colorbar(self.cax, ticks=[0,125,250]).set_label('Ice concentration in %')
	...
	def calcday(self,delta):	
		...
		return '{}{}{}'.format(self.year,stringmonth,stringday)
