...
class Data_restructure:
	...
	def createSuperlist (self):
		...
		for year in range(1979,2023):
			AWP_mean = ['A', 'B', 'C', 'D', 'E']
			Climatedata = pd.read_csv(f'Melt_AWP/csv/_AWP_{year}.csv', names=AWP_mean,header=0)
	...
	def createSuperRegionList (self):
		...
		for year in range(1979,2023):
			AWP_mean = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M']
			Climatedata = pd.read_csv(f'Melt_AWP/csv/_AWP_regional_{year}.csv', names=AWP_mean,header=0)
			column1 = Climatedata.A.tolist()
			column2 = Climatedata.B.tolist()
			column3 = Climatedata.C.tolist()
			column4 = Climatedata.D.tolist()
...
#			self.appendSuperRegionList(data)
		...
		self.csvexport('Melt_AWP/AWP_region_Heatmap.csv',[self.SoO,self.Bers,self.HB,self.BB,self.EG,
					 self.BaS,self.KS,self.LS,self.ES,self.CS,self.BeaS,self.CA,self.AB])
	
	def regionExcelbook_whole_Arctic (self):
		...
		inputfile = 'Melt_AWP/csv/_AWP_{}.csv'
		outputfile = 'Melt_AWP/AWP_new.xlsx'
...
#			print(Climatedata.B[0])
	...
	def regionExcelbook_Daily_by_year (self):
		with pd.ExcelWriter('Melt_AWP/AWP_by_Year_regional_Daily.xlsx') as writer:
			for year in range(1979,2023):
				Climatedata = pd.read_csv('Melt_AWP/csv/_AWP_regional_daily_{}.csv'.format(year))
				Climatedata.to_excel(writer, sheet_name=str(year),index=False)
				
	def regionExcelbook_Accu_by_year (self):
		with pd.ExcelWriter('Melt_AWP/AWP_by_Year_regional_Accu.xlsx') as writer:
			for year in range(1979,2023):
				Climatedata = pd.read_csv('Melt_AWP/csv/_AWP_regional_{}.csv'.format(year))
				Climatedata.to_excel(writer, sheet_name=str(year),index=False)
...
#		
		if mode == 'Accu':
			inputfile = 'Melt_AWP/csv/_AWP_regional_{}.csv'
			outputfile = 'Melt_AWP/AWP_by_region.xlsx'
		elif mode == 'Daily':
			inputfile = 'Melt_AWP/csv/_AWP_regional_daily_{}.csv'
			outputfile = 'Melt_AWP/AWP_by_region_Daily.xlsx'
...
#		
		if mode == 'Accu':
			inputfile = 'Melt_AWP/csv/_AWP_regional_{}.csv'
			outputfile = 'Melt_AWP/AWP_by_region_decades.xlsx'
		elif mode == 'Daily':
			inputfile = 'Melt_AWP/csv/_AWP_regional_daily_{}.csv'
			outputfile = 'Melt_AWP/AWP_by_region_Daily_decades.xlsx'
		...
		if mode == 'decades':
			inputfile = 'Melt_AWP/csv/_AWP_regional_{}.csv'
			outputfile = 'Melt_AWP/2020mean_Melt_AWP_by_region.xlsx'
