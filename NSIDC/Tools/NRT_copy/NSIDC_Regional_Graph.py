...
class Graph_Creater:
    ...
    def area_graph(self,df,df_NRT,sheet_name,xxx):
        with plt.style.context('dark_background'):
            ...
            ax.text(0.7, -0.06, 'cryospherecomputing.com/regional',transform=ax.transAxes,color='grey', fontsize=10)
            
            maximum = max(df['1980s']*1.1)
            ax.grid(True)
        
            plt.plot( df['1980s'], color=(0,0.1,1),label='1980s',lw=2,ls='--')
            plt.plot( df['1990s'], color=(0,0.5,0.1),label='1990s',lw=2,ls='--')
            plt.plot( df['2000s'], color=(0.5,0,0.8),label='2000s',lw=2,ls='--')
            plt.plot( df['2010s'], color=(0.8,0.5,0),label='2010s',lw=2,ls='--')
            plt.plot( df['Min'], color=(0.7,0,0),label='Minimum',lw=1,ls='--')
            plt.plot( df_NRT, color=(0.9,1,1),label=f'{self.year}',lw=2)
        
            plt.axis([0,366,0,maximum])
            plt.legend(loc=1, shadow=True, fontsize='medium')
            fig.tight_layout(pad=1)
            fig.subplots_adjust(top=0.95)
            fig.savefig(f'{self.webpath}/NSIDC_Area/Regional/Area_{xxx}.png')
...
#         plt.show()
    ...
    def extent_graph(self,df,df_NRT,sheet_name,xxx):
        with plt.style.context('dark_background'):
            ...
            transform=ax.transAxes,
            color='grey', fontsize=10)
            ax.text(0.66, -0.06, 'cryospherecomputing.com/regional-extent',transform=ax.transAxes,color='grey', fontsize=10)
            ...
            fig.tight_layout(pad=1)
            fig.subplots_adjust(top=0.95)
            fig.savefig(f'{self.webpath}/NSIDC_Area/Regional/Extent_{xxx}.png')
        #plt.show()
        plt.close()
        return
    
    def loadRegionaldata(self):
        df_NRT = pd.read_csv(f'{self.webpath}/NSIDC_Area/Data/Regional_NRT.csv')
        column_names = df_NRT.columns
        
        excelfile = f'{self.webpath}/NSIDC_Area/Data/Regional_Climate.xls'
    ...
    def loadRegionaldata_extent(self):
        df_NRT = pd.read_csv(f'{self.webpath}/NSIDC_Area/Data/Regional_NRT_extent.csv')
        column_names = df_NRT.columns
        
        excelfile = f'{self.webpath}/NSIDC_Area/Data/Regional_Climate_extent.xls'
        for xxx in range(0,13):    #13 regional
            df = pd.read_excel(excelfile,sheet_name=xxx)
