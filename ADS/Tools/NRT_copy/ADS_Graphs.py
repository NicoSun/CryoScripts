...
#		filename = 'Binary/AMSR2_SIT_{}{}{}.dat'.format(self.year,self.stringmonth,self.stringday)
		...
		for image in range(1,day+1):
			stringday = str(image).zfill(2)
			file_name = f'AMSR2_SIT_{year}{self.stringmonth}{stringday}.dat'
			iceread = CryoIO.openfile(os.path.join(filepath,file_name),np.uint16)

			thicknesslist = np.ma.masked_greater(iceread,400)
			histogram = np.histogram(thicknesslist,bins=16,range=(20,400))
			
			monthlist[file_name[14:-4]] = histogram[0]
...
#		plt.show()
	...
	def makemovie(self):
		...
		for image in range(1,day+1):
			stringday = str(image).zfill(2)
			image_name = f'AMSR2_SIT_{year}-{self.stringmonth}-{stringday}.png'
			image_name_anom = f'AMSR2_SIT_Anomaly_{year}-{self.stringmonth}-{stringday}.png'
			image_list.append(os.path.join(filepath,image_name))
			image_list_anom.append(os.path.join(filepath_anom,image_name_anom))
