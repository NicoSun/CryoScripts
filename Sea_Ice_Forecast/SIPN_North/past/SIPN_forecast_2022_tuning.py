...
class NSIDC_prediction:
	...
	def __init__  (self):
		self.filepath = '/media/nico/REVIVA/HomeServer/Cryoscripts/NSIDC'
		self.filepath_SIPN = '/media/nico/REVIVA/HomeServer/Cryoscripts/Sea_Ice_Forecast'
	...
	def prediction(self):
		...
		iceforecast = CryoIO.openfile(f'{self.filepath}/DataFiles/{filename}',np.uint8)/250
# 		icedrift_error = np.zeros(len(self.regmaskf))
		...
		for count in range (self.index,countmax,1):
			filename = f'{self.filepath}/DataFiles/NSIDC_{self.datestring}.bin'
			filenameChange = f'{self.filepath_SIPN}/DataFiles/Forecast_SIC_change/NSIDC_SIC_Change_{self.stringmonth}{self.stringday}.bin'
			...
			CryoIO.savenumpy('analysis/predict/SIPN2_{self.datestring}.npz',sicmap)
...
def spawnprocess(datalist):
	...
	area,extent = action.getvolume(datalist[1],122,1,6,datalist[0]) #daycount: 122 until 30th Sep, 153 until Oct
