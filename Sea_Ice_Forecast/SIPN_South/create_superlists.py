...
class NSIDC_area:
	...
	def __init__  (self):
		...
		self.forecasttype = '002' # Low: 001 Mean: 002 High: 003
		self.columnA = []
		self.columnB = []
		self.columnC = []
		self.columnD = []
	...
	def loadyearcsvdata (self):
		for yearload in range(2001,2019):
			AWP_mean = ['A', 'B', 'C', 'D']
			Climatedata = pd.read_csv('2019-20_submission/_SIPN_forecast_{}_{}.csv'.format(self.forecasttype,yearload), names=AWP_mean,header=0)
