...
class NSIDC_analysis:
    ...
    def compute_error(self):
        ...
        if self.Cdate.year == 'Mean_00_19':
            filename_pre = f'{self.filepath_SIPN }/DataFiles_s/Mean_00_19/NSIDC_Mean_{self.Cdate.strMonth}{self.Cdate.strDay}_south.npz'
        elif self.Cdate.year == 'Forecast_Mean':
            filename_pre = f'{self.filepath_SIPN }/DataFiles_s/Forecast_Mean/NSIDC_Mean_{self.Cdate.strMonth}{self.Cdate.strDay}.npz'
        else:
            filename_pre = f'{self.filepath}/DataFiles/{self.Cdate.year}/NSIDC_{self.Cdate.datestring}_south.npz'
        
        
        filename_obs = f'{self.filepath}/DataFiles/{self.yearref}/NSIDC_{self.datestringref}_south.npz'
...
def yearloop(yearStart,yearEnd):
    for i in range(yearStart,yearEnd):
        ...
        CryoIO.csv_rowexport(f'correlation_data/{mode}_{i}_overview.csv',overview)
