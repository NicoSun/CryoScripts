		...
		for x in range (0,arraylength):
			if self.regmaskf[x] < 11:
				if self.latmaskf[x] < -56:
					...
					if self.latmaskf[x] > -70: #higher correction in Weddell Sea
						latfactor = 2
					else:
						latfactor = 1
					driftcorrection = basedriftcorrection*latfactor
					if abs(icedrift_error[x]) > 0.8:
						aaa = 3
					else:
						aaa = 1
					driftcorrection = driftcorrection*aaa
					if icearray[x] < 0.1 and icedrift_error[x] < 0: #0.1,0
						driftcorrection = driftcorrection *2 # icefree area accelerated re-freeze
					elif icearray[x] > 0.44 and icedrift_error[x] < 0: #0.1,0
						driftcorrection = driftcorrection *0.2 # already refrozen, but still error
					elif icearray[x] < 0.5 and icedrift_error[x] > 0.3: # 0.5,0
						driftcorrection = driftcorrection*0.75 # thin ice slow down melt if overpredict ice
...
action = NSIDC_prediction()
action.getvolume(1.5,91,30,11,2020)
