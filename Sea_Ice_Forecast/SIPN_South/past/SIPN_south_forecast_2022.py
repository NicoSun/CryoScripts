...
#		self.map_create()
	...
	def prediction(self):
		...
		for count in range (self.index,countmax,1):
			...
			filenameAvg = f'{self.filepath_SIPN}/DataFiles_s/Forecast_Mean/NSIDC_Mean_{self.stringmonth}{self.stringday}.npz'
			filenameChange = f'{self.filepath_SIPN}/DataFiles_s/Forecast_SIC_change/NSIDC_SIC_Change_{self.stringmonth}{self.stringday}.npz'
			filenameStdv = f'{self.filepath_SIPN}/DataFiles_s/Forecast_Stdv/NSIDC_Stdv_{self.stringmonth}{self.stringday}.npz'
			...
			bottommeltLow = self.bottommelt * 1.15
...
#				self.early_recalibrate(iceforecast)
				...
				iceforecastHigh = np.array(iceforecastMean + iceStdv*0.20)
				iceforecastLow = np.array(iceforecastMean - iceStdv*0.28)
...
# 			if count >= (countmax-30):
			if count >330:
# 				visualize.thicknessshow(self.iceMean,int(calcvolume),int(extent),'Mean',self.datestring)
				visualize.concentrationshow(sicmap,int(area),int(extent),'Mean',self.datestring)
				visualize.fig2.savefig(f'Images/{self.datestring}')
...
# 			CryoIO.savebinaryfile(f'temp/001/SIPN2_SIT_001_{self.datestring}.bin',iceLastDateLow)
# 			CryoIO.savebinaryfile(f'temp/002//SIPN2_SIT_002_{self.datestring}.bin',iceLastDateMean)
# 			CryoIO.savebinaryfile(f'temp/003/SIPN2_SIT_003_{self.datestring}.bin',iceLastDateHigh)
# =============================================================================
			
			
			# save binary SIC maps
			CryoIO.savebinaryfile(f'temp/001/SIPN2_SIC_001_{self.datestring}.bin',sicmapLow)
			CryoIO.savebinaryfile(f'temp/002/SIPN2_SIC_002_{self.datestring}.bin',sicmap)
			CryoIO.savebinaryfile(f'temp/003/SIPN2_SIC_003_{self.datestring}.bin',sicmapHigh)
...
#			print(count)
	...
	def csvexport_by_forecasttype(self):
		...
		exportpath = 'temp/'
		CryoIO.csv_columnexport(f'{exportpath}_SIPN_forecast_002_{self.year}.csv',
				[self.CSVDatum,self.CSVVolume,self.CSVArea,self.CSVExtent])
		CryoIO.csv_columnexport(f'{exportpath}_SIPN_forecast_003_{self.year}.csv',
				[self.CSVDatum,self.CSVVolumeHigh,self.CSVAreaHigh,self.CSVExtentHigh])
		CryoIO.csv_columnexport(f'{exportpath}_SIPN_forecast_001_{self.year}.csv',
...
# =============================================================================
		CryoIO.csv_columnexport(f'{exportpath}__SIPN_forecast_Area_{self.year}.csv',
				[self.CSVDatum,self.CSVAreaLow,self.CSVArea,self.CSVAreaHigh])
		CryoIO.csv_columnexport(f'{exportpath}__SIPN_forecast_Extent_{self.year}.csv',
	...
	def getvolume (self,thickness,daycount,day,month,year):
		...
		filename = 'temp/NSIDC_20221201_south.bin'
		...
		for x in range (0,len(self.iceLastDate)):
			if self.regmaskf[x] < 7:
				if 0.25 < self.iceLastDate[x] <= 1 and self.latmaskf[x] < -55:
					self.iceLastDate[x] = thickness*self.iceLastDate[x] * (self.latmaskf[x]/(-72))
